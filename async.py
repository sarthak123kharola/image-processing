...
face_cascade = initialize_haar_cascade('D:\image processinghaarcascade_frontalface_default.xml')
...
def apply_filter(filter_type):
    global img, processed_img
    temp = img  # Store the original image reference
    img = processed_img  # Ensure you are working on the processed image

    if img is None:
        messagebox.showwarning("Warning", "No image loaded.")
        return

    # Ensure the image is in a compatible format (BGR for color filters)
    if len(img.shape) == 2 and filter_type not in ['Grayscale', 'Invert', 'Canny', 'Edge Detection']:
    ...
    if filter_type == 'Blur':
        ...
        # Ensure the image is in BGR for Sepia filter
        if len(img.shape) == 2:
            img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
...
def run_model_in_thread(model_function, *args):
    def wrapper():
        with lock:  # Acquire the lock before running the model function
...
def process_queue():
    try:
        while not result_queue.empty():
            ...
            # For example: label.config(text=result)
    except queue.Empty:
        pass
    root.after(100, process_queue)  # Continue polling the queue
...
operations = [
    ("Visualize", lambda: run_model_in_thread(visualize_histogram)),
    ("Visualize Brightness Distribution", lambda: run_model_in_thread(visualize_brightness_distribution)),
    ("Visualize Color Correlation", lambda: run_model_in_thread(visualize_color_correlation)),
    ("Visualize Dominant Colors", lambda: run_model_in_thread(visualize_dominant_colors, 3)),
    ("Visualize Pixel Intensity", lambda: run_model_in_thread(visualize_pixel_intensity_across_regions)),
    ("Visualize Rgb Channel Comparision", lambda: run_model_in_thread(visualize_rgb_channel_comparison)),
